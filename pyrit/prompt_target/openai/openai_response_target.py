...
class OpenAIResponseTarget(OpenAITarget):
    ...
    def fresh_instance(
        ...
        *,
        ...
        Args:
            extra_body_parameters (dict[str, Any] | None): Optional overrides for the
                extra body parameters of the new instance. Merged onto the stored values.
            grammar_name (str | None): Optional override for the grammar name of the
                new instance.
        ...
        Raises:
            ValueError: If ``grammar_name`` is provided without exactly one grammar tool.
        """
        ...
        if grammar_name is not None:
            body_parameters = init_args.get("extra_body_parameters") or {}
            grammar_tools = [
                tool
                for tool in body_parameters.get("tools", [])
                if tool.get("type") == "custom" and tool.get("format", {}).get("type") == "grammar"
            ]
            if len(grammar_tools) != 1:
                raise ValueError("grammar_name override requires exactly one grammar tool.")
            grammar_tools[0]["name"] = grammar_name
